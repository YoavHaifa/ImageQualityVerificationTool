...
void CTentScorer::ComputeLocalMaxScore(int iRing)
{
	...
	// Go back down as long as available
	while (iPrev > 1 && mvRingMean[iPrev - 1] < mvRingMean[iPrev])
		iPrev--;

	// Go forward down as long as available
	while (iNext < mnRings - 2 && mvRingMean[iNext + 1] < mvRingMean[iNext])
		iNext++;
